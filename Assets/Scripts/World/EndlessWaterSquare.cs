...
public class EndlessWaterSquare : MonoBehaviour {
    ...
    {
        //The center piece
        GameObject center = AddWaterPlane(0f, 0f, 0f, squareWidth, innerSquareResolution);
        ...
        for (int x = -2; x <= 2; x += 1)
        {
            for (int z = -2; z <= 2; z += 1)
            {
                ...
                if (x == 0 && z == 0)
                {
                    ...
                    center.GetComponent<BoxCollider>().center = new Vector3(500, 0, 500);
                    center.GetComponent<BoxCollider>().isTrigger = true;
                    center.GetComponent<BoxCollider>().size = new Vector3(1000,0,1000);
                    continue;
                }
    ...
    //Add one water plane
    GameObject AddWaterPlane(float xCoord, float zCoord, float yPos, float squareWidth, float spacing)
    {
