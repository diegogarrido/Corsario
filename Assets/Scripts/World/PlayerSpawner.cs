...
public class PlayerSpawner : MonoBehaviour
{
    ...
    public void LoadData()
    {
        ...
        else
        {
            boat = 0;
            world.player = Instantiate(boats[boat]);
            world.playerCoord = new int[] { 3, 3 };
            Vector3 position = new Vector3((3 * world.spacing) + 1, 0, (3 * world.spacing) + 1);
            world.player.gameObject.transform.position = position;
        }
        if (DataManager.MapCreated())
        {
            for (int i = world.playerCoord[0] - 3; i < world.playerCoord[0] + 3; i++)
            {
                for (int j = world.playerCoord[1] - 3; j < world.playerCoord[1] + 3; j++)
                {
